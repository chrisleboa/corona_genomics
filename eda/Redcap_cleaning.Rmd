...
output_file <- "/Users/ChrisLeBoa/GitHub/corona_genomics/data/clean_travel_vars_20200612.csv"
```
...
occupation_types <- 
  ...
  mutate(
    occupation_type = case_when(
      ...
      str_detect(occupation, "teacher|professor|school") == TRUE ~ "school employee",
      str_detect(occupation, "doctor|nurse|medical") == TRUE ~ "healthcare worker", 
      str_detect(occupation, "airline|flight|pilot") == TRUE ~ "airline worker", 
      str_detect(occupation, "fire|police|emt|paramedic") == TRUE ~ "first responder", 
      str_detect(occupation, "none|housewife|unemployed|mom|not currently employed") == TRUE ~ "not working",
      is.na(occupation) == TRUE ~ NA_character_,
      TRUE ~ "ohter_professional"), 
  ) 
...
travel_locations <- 
  ...
  mutate(
    ...
    travel_europe = case_when(  #This variable needs some work
      str_detect(location_trip1_location, "France|Spain|UK|London|Belgium|Portugual|Italy|Ireland") == TRUE ~ 1,
      str_detect(location_trip2_location, "France|Spain|UK|London|Belgium|Portugual|Italy|Ireland") == TRUE ~ 1,
      str_detect(location_trip3_location, "France|Spain|UK|London|Belgium|Portugual|Italy|Ireland") == TRUE ~ 1,
      TRUE ~ 0                                          
    ), 
   ...
   travel_other_us = case_when(  #This variable needs some work
      str_detect(location_trip1_location, "Nevada|NV|TX|OH|FL|NJ|Idaho|Utah|South Dakota") == TRUE ~ 1,
      str_detect(location_trip2_location, "Nevada|NV|TX|OH|FL|NJ|Idaho|Utah|South Dakota") == TRUE ~ 1,
      str_detect(location_trip3_location, "Nevada|NV|TX|OH|FL|NJ|Idaho|Utah|South Dakota") == TRUE ~ 1,
      TRUE ~ 0                                          
    ), 
  )
...
address_check <- 
  ...
  mutate(
    ...
    address_use = 
     ...
     str_detect(address_use, "3354 Brichar Way") == TRUE 
...
```{r}
address_check %>% 
  count(travel_california_los_angeles)
